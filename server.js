...
app.post('/api/projects', (req, res) => {
  ...
  const now = ts();
  const id  = DB._nextProjectId++;
  const project = {
    id, name: name.trim(), status: status || 'not_started',
    context: '', next_step: '', order_idx: id,
    created_at: now, updated_at: now, tasks: [],
  };
...
app.put('/api/tasks/:id', (req, res) => {
  ...
  const now = ts();
  const { title, done, project_id, due_date } = req.body;
...
app.get('/api/plan', (req, res) => {
  ...
  const overdue  = [], today = [], tomorrow = [], week = [];
  for (const t of allTasks) {
    const d = t.due_date;
    if (d < todayStr)    overdue.push(t);
    else if (d === todayStr) today.push(t);
    else if (d === tomStr)   tomorrow.push(t);
    else if (d <= weekEndStr) week.push(t);
  }
  res.json({ overdue, today, tomorrow, week });
});
